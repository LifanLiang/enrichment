...
#### Comparison of heritability in Signac (joint analysis) in schizophrenia (SCZ)
```{r, echo=F}
...
#### Dynamic elements vs. static elements from Signac (marginal analysis) in schizophrenia (SCZ)
```{r, echo=F}
enrich.ld <- read.table("data/lexi_18lines_res/PGC3_dyn_vs_stat_joint.results", sep="\t", header=T, row.names=1)
temp <- enrich.ld[(nrow(enrich.ld)-8):nrow(enrich.ld),]
rownames(temp) <- annot
