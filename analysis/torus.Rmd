...
#### DAP vs. static elements from Signac (marginal analysis)
...
#### DAP vs. static elements from Signac (joint analysis)
...
#### DAP vs. static elements from pseudo bulk (marginal analysis)
...
table <- rbind(enrich.dap, enrich.static.pseu)
table[,2:4] <- table[,2:4] * log2(exp(1))
table$annotation <- factor(table$annotation, levels=table$annotation)
plot_or(table[-c(5,6,11),], "", xlab = expression(paste(log[2], ' fold enrichment of SZ variants')), ylab = "Functional annotations")
```

#### DAP vs. static elements from pseudo bulk (joint analysis)
...
enrich.joint.pseu <- read.table("data/joint_dynamic/joint_dynamic_vs_static_pseudobulk.tsv", header=T)
colnames(enrich.joint.pseu)  = c("annotation", "logOR", "lower", "upper")
table <- enrich.joint.pseu[-c(1,4,10,11),]
table[,2:4] <- table[,2:4] * log2(exp(1))
table$annotation <- factor(table$annotation, levels=table$annotation)
plot_or(table, "", xlab = expression(paste(log[2], ' fold enrichment of SZ variants')), ylab = "Functional annotations")
```